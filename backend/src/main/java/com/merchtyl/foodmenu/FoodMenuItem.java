...
public class FoodMenuItem extends BaseUuidEntity {
    ...
    FoodMenuItem(Store store, FoodMenuCategory category, Product product, String displayName, BigDecimal price, int displayOrder, boolean available, String imageUrl) { this.store=store; this.tenantId=store.getTenantId(); this.product=product; update(category, displayName, price, displayOrder, available, imageUrl); }
    void update(FoodMenuCategory category, String displayName, BigDecimal price, int displayOrder, boolean available, String imageUrl) { this.category=category; this.displayName=displayName.trim(); this.price=price; this.displayOrder=displayOrder; this.available=available; this.imageUrl=imageUrl == null || imageUrl.isBlank() ? null : imageUrl.trim(); }
    public Store getStore(){return store;} public UUID getTenantId(){return tenantId;} public FoodMenuCategory getCategory(){return category;} public Product getProduct(){return product;} public String getDisplayName(){return displayName;} public BigDecimal getPrice(){return price;} public int getDisplayOrder(){return displayOrder;} public boolean isAvailable(){return available;} public String getImageUrl(){return imageUrl;}
}
