...
public class FoodMenuItem extends BaseUuidEntity {
    ...
    @OneToMany(mappedBy="menuItem",cascade=CascadeType.ALL,orphanRemoval=true) @OrderBy("displayOrder") private Set<FoodMenuItemModifierGroupAssignment> modifierGroupAssignments=new LinkedHashSet<>();
    ...
    void clearSelections(){variants.clear();modifierGroupAssignments.clear();components.clear();}
    void addSelections(List<FoodMenuDtos.VariantRequest> variantValues,List<FoodMenuItemModifierGroupAssignment> assignments,List<FoodMenuDtos.ComponentRequest> componentValues){variantValues.forEach(v->variants.add(new FoodMenuItemVariant(this,v.name(),v.price(),v.displayOrder(),v.available())));modifierGroupAssignments.addAll(assignments);componentValues.forEach(c->components.add(new FoodMenuItemComponent(this,c.name(),c.includedByDefault(),c.removable(),c.allowExtra(),c.extraPrice(),c.displayOrder(),c.active())));}
    FoodMenuItemModifierGroupAssignment assign(FoodMenuModifierGroup group,int min,int max,int order,boolean active){return new FoodMenuItemModifierGroupAssignment(this,group,min,max,order,active);}
    public Store getStore(){return store;} public UUID getTenantId(){return tenantId;} public FoodMenuCategory getCategory(){return category;} public Product getProduct(){return product;} public boolean isLinkedProduct(){return linkedProduct;} public String getDisplayName(){return displayName;} public String getDescription(){return description;} public BigDecimal getPrice(){return price;} public int getDisplayOrder(){return displayOrder;} public boolean isAvailable(){return available;} public String getImageUrl(){return imageUrl;} public Set<FoodMenuItemVariant> getVariants(){return variants;} public Set<FoodMenuItemModifierGroupAssignment> getModifierGroupAssignments(){return modifierGroupAssignments;} public Set<FoodMenuItemComponent> getComponents(){return components;}
}
