...
public final class FoodMenuDtos {
    ...
    public record ItemRequest(UUID productId, @NotNull UUID categoryId, @NotBlank @Size(max=180) String displayName, @Size(max=1000) String description, @NotNull @DecimalMin("0.00") BigDecimal price, UUID taxCategoryId, @PositiveOrZero int displayOrder, boolean available, @Size(max=1000) String imageUrl, List<@Valid VariantRequest> variants, List<@Valid ModifierGroupRequest> modifierGroups) { public ItemRequest(UUID productId,UUID categoryId,String displayName,String description,BigDecimal price,UUID taxCategoryId,int displayOrder,boolean available,String imageUrl){this(productId,categoryId,displayName,description,price,taxCategoryId,displayOrder,available,imageUrl,List.of(),List.of());} }
    ...
    public record ItemResponse(UUID id, UUID storeId, UUID categoryId, String categoryName, UUID productId, String productName, String displayName, String description, BigDecimal price, boolean inventoryTracked, boolean madeToOrder, int displayOrder, boolean available, String imageUrl, List<VariantResponse> variants,List<ModifierGroupResponse> modifierGroups,long version) { static ItemResponse from(FoodMenuItem value){return new ItemResponse(value.getId(), value.getStore().getId(), value.getCategory().getId(), value.getCategory().getName(), value.isLinkedProduct()?value.getProduct().getId():null, value.isLinkedProduct()?value.getProduct().getName():null, value.getDisplayName(), value.getDescription(), value.getPrice(), value.isLinkedProduct()&&value.getProduct().isInventoryTrackingEnabled(), !value.isLinkedProduct(), value.getDisplayOrder(), value.isAvailable(), value.getImageUrl(),value.getVariants().stream().map(VariantResponse::from).toList(),value.getModifierGroups().stream().map(ModifierGroupResponse::from).toList(), value.getVersion());} }
