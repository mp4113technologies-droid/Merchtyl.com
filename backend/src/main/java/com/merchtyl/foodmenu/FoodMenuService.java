...
public class FoodMenuService {
    ...
    @Transactional public ItemResponse createItem(UUID storeId, ItemRequest request, Authentication auth) { var store=requireStore(storeId,auth); boolean linked=request.productId()!=null; if(linked)requireUniqueItem(storeId,request.productId(),null); Product product=linked?linkedProduct(store.getTenantId(),request.productId()):backingProduct(store,request); return ItemResponse.from(items.save(new FoodMenuItem(store,category(storeId,request.categoryId()),product,linked,request.displayName(),request.description(),request.price(),request.displayOrder(),request.available(),request.imageUrl()))); }
    ...
    private Product backingProduct(com.merchtyl.store.Store store,ItemRequest request){
        ...
        product.assignTenant(store.getTenantId());product.markRestaurantMenuManaged();products.save(product);
        var mapping=new StoreProduct(store.getTenantId(),store,product);mapping.update(new StoreProductRequest(store.getId(),true,true,request.price(),BigDecimal.ZERO,null,null,true,false));storeProducts.save(mapping);return product;
    }
