...
public class SubscriptionEntitlementService {
    ...
    @Transactional public void deactivateIfUnused(UUID tenantId,CommercialCapability capability){
        int usage=capability==CommercialCapability.FOOD_SERVICE?Objects.requireNonNull(jdbc.queryForObject("select count(*) from stores store join store_capabilities capability on capability.store_id=store.id where store.tenant_id=? and store.active=true and capability.capability='FOOD_SERVICE'",Integer.class,tenantId)):0;
