...
public class InitialInventoryWorkbookService {
    public static final int TEMPLATE_VERSION=1, MAX_ROWS=5000;
    public static final long MAX_FILE_BYTES=10L*1024*1024;
    private static final String[] HEADERS={"Product Reference","Product Name *","Variant Name *","Barcode","SKU","Selling Price *","Cost Price","Opening Quantity *","Low Stock Level","Category","Brand","Unit","Tax Category *","Supplier","Age Restricted","Active"};
    ...
    public byte[] template(Store store,String merchantName){
        try(var workbook=new XSSFWorkbook();var out=new ByteArrayOutputStream()){
            ...
            addDropdown(setup,workbook,9,0);addDropdown(setup,workbook,10,1);addDropdown(setup,workbook,11,2);addDropdown(setup,workbook,12,3);addDropdown(setup,workbook,13,4);addListDropdown(setup,14,"YES,NO");addListDropdown(setup,15,"YES,NO");
    ...
    public List<RawRow> parse(byte[] bytes){
        ...
        try(var workbook=new XSSFWorkbook(new ByteArrayInputStream(bytes))){
            ...
            for(int i=1;i<=sheet.getLastRowNum();i++){Row row=sheet.getRow(i);if(row==null||blankRow(row,formatter))continue;result.add(new RawRow(i+1,text(row,0,formatter),text(row,1,formatter),text(row,2,formatter),text(row,3,formatter),formulaSafeText(row,4,formatter),decimal(row,5,formatter),decimal(row,6,formatter),decimal(row,7,formatter),decimal(row,8,formatter),text(row,9,formatter),text(row,10,formatter),text(row,11,formatter),text(row,12,formatter),text(row,13,formatter),bool(row,14,formatter),bool(row,15,formatter)));}
