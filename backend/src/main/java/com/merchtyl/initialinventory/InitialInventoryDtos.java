...
public final class InitialInventoryDtos {
    ...
    public record RawRow(int rowNumber,String productReference,String productName,String variantName,String barcode,String additionalBarcodes,String sku,
    ...
    public record RowPreview(int rowNumber,String productReference,String productName,String variantName,String barcode,String additionalBarcodes,String sku,
                             BigDecimal sellingPrice,BigDecimal openingQuantity,List<String> actions,List<String> errors) {
        static RowPreview from(InitialInventoryImportRow row){return new RowPreview(row.getRowNumber(),row.getProductReference(),row.getProductName(),row.getVariantName(),row.getBarcode(),row.getAdditionalBarcodes(),row.getSku(),row.getSellingPrice(),row.getOpeningQuantity(),List.of(row.getProductAction(),row.getVariantAction(),"ASSIGN_PRODUCT_TO_STORE","CREATE_OPENING_INVENTORY"),row.errors());}
    }
