...
class FoodMenuServiceTest {
    ...
    @Test void createsMadeToOrderItemWithoutRetailProductOrInventory(){var store=mock(Store.class);var storeId=UUID.randomUUID();var category=mock(FoodMenuCategory.class);when(store.getId()).thenReturn(storeId);when(store.getTenantId()).thenReturn(UUID.randomUUID());when(capabilityService.requireCapability(storeId,StoreCapability.FOOD_SERVICE,authentication)).thenReturn(store);when(categories.findByIdAndStoreId(any(),eq(storeId))).thenReturn(Optional.of(category));when(products.save(any())).thenAnswer(invocation->invocation.getArgument(0));when(items.save(any())).thenAnswer(invocation->invocation.getArgument(0));var result=service.createItem(storeId,new ItemRequest(null,UUID.randomUUID(),"Samosa","Vegetable samosa",new BigDecimal("2.49"),null,1,true,null),authentication);assertThat(result.madeToOrder()).isTrue();assertThat(result.inventoryTracked()).isFalse();assertThat(result.productId()).isNull();verify(storeProducts).save(any(StoreProduct.class));}
