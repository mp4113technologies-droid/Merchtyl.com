...
export function FoodPosPage() {
  ...
  return (
    <Stack spacing={2} sx={{ minHeight: 'calc(100vh - 88px)' }}>
      <Stack direction="row" alignItems="center" spacing={1}><RestaurantIcon color="primary" fontSize="large" /><Box><Typography variant="h4">{configuration.data?.kitchenDisplayName ?? 'Restaurant / Kitchen POS'}</Typography><Typography color="text.secondary">{store?.name}</Typography></Box></Stack>
