...
import {
  Alert, Box, Button, Checkbox, Chip, Dialog, DialogActions, DialogContent, DialogTitle, FormControl, FormControlLabel, Radio, RadioGroup,
...
export function PlatformPricingPlansPage() {
  ...
  const [selected, setSelected] = React.useState<PricingPlan | null>(null); const [applicationMode,setApplicationMode]=React.useState('NEXT_BILLING_CYCLE'); const [effectiveDate, setEffectiveDate] = React.useState(localDateInputValue(new Date(Date.now()+86400000))); const [confirmRemoval,setConfirmRemoval]=React.useState(false); const [confirmImmediate,setConfirmImmediate]=React.useState(false);
  ...
  const schedule = useMutation({ mutationFn: async () => schedulePlatformPricingVersion(await token(), selected!.id, { pricing: form, applicationMode, effectivePolicy:applicationMode==='SCHEDULED'?'SPECIFIC_FUTURE_DATE':'NEXT_BILLING_CYCLE', effectiveDate:applicationMode==='SCHEDULED'?effectiveDate:null, existingSubscriberPolicy: applicationMode==='NEXT_BILLING_CYCLE'?'APPLY_NEXT_BILLING_CYCLE':'NEW_SUBSCRIPTIONS_ONLY', confirmCapabilityRemoval: confirmRemoval, expectedPlanVersion: selected!.version }), onSuccess: async () => { setConfirmImmediate(false); setOpen(false); await client.invalidateQueries({queryKey:['platform-pricing-history']}); await client.invalidateQueries({queryKey:['platform-pricing-plans']}); await client.invalidateQueries({queryKey:['platform-pricing-plan-options']}); await client.invalidateQueries({queryKey:['platform-tenants-billing']}); } });
  ...
  return <Stack spacing={3}><Header title="Pricing Plans" action={<Button startIcon={<AddIcon />} variant="contained" onClick={openCreate}>New Plan</Button>} />
    {save.error || update.error || schedule.error || cancel.error ? <Alert severity="error">{getApiErrorMessage(save.error??update.error??schedule.error??cancel.error)}</Alert> : null}
    ...
    <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="md"><DialogTitle>{selected?'Edit Pricing':'Create Pricing Plan'}</DialogTitle><DialogContent><Grid container spacing={2} sx={{ mt: 0 }}>
      ...
      {selected?<><Grid item xs={12}><Typography variant="h6">Apply Changes</Typography><Typography color="text.secondary">When should this pricing change take effect?</Typography></Grid><Grid item xs={12}><RadioGroup value={applicationMode} onChange={event=>setApplicationMode(event.target.value)}><FormControlLabel value="IMMEDIATE" control={<Radio />} label={<Box><Typography>Apply immediately</Typography><Typography variant="caption" color="text.secondary">Merchants on this plan begin using the new configuration now. No mid-cycle prorated invoice is created.</Typography></Box>} /><FormControlLabel value="NEXT_BILLING_CYCLE" control={<Radio />} label={<Box><Typography>Next billing cycle</Typography><Typography variant="caption" color="text.secondary">Existing merchants keep their assigned version until renewal; new merchants use the published version.</Typography></Box>} /><FormControlLabel value="SCHEDULED" control={<Radio />} label={<Box><Typography>Schedule date</Typography><Typography variant="caption" color="text.secondary">Apply to subscribed merchants on a future server-authoritative date.</Typography></Box>} /></RadioGroup></Grid><Grid item xs={12} sm={6}>{applicationMode==='SCHEDULED'?<TextField fullWidth label="Effective Date" type="date" InputLabelProps={{shrink:true}} inputProps={{min:localDateInputValue(new Date(Date.now()+86400000))}} value={effectiveDate} onChange={e=>setEffectiveDate(e.target.value)}/>:null}</Grid><Grid item xs={12}><Alert severity="info">Pricing Change Summary: Base subscription {money(selected.basePrice,selected.currency)} → {money(Number(form.basePrice),selected.currency)}; additional store {money(selected.additionalStorePrice??0,selected.currency)} → {money(Number(form.additionalStorePrice??0),selected.currency)}; registers/store {selected.includedRegisters??0} → {Number(form.includedRegisters??0)}; additional register {money(selected.additionalRegisterPrice??0,selected.currency)} → {money(Number(form.additionalRegisterPrice??0),selected.currency)}. Affected active subscriptions: {selected.activeMerchants}. Historical invoices are never recalculated.</Alert></Grid><Grid item xs={12}><FormControlLabel control={<Checkbox checked={confirmRemoval} onChange={e=>setConfirmRemoval(e.target.checked)}/>} label="Confirm capability removals for affected subscriptions" /></Grid><Grid item xs={12}><Typography variant="h6">Pricing History</Typography></Grid>{history.data?.map(version=><Grid item xs={12} key={version.id}><Paper variant="outlined" sx={{p:1}}><Stack direction="row" justifyContent="space-between"><Box><Typography>Version {version.versionNumber} · {version.status} · effective {version.effectiveFrom}</Typography><Typography variant="caption">Base {money(version.pricing.basePrice,version.pricing.currency)} · additional store {money(version.pricing.additionalStorePrice??0,version.pricing.currency)} · {version.pricing.includedRegisters??0} registers/store · additional register {money(version.pricing.additionalRegisterPrice??0,version.pricing.currency)} · {version.subscriberPolicy.replaceAll('_',' ')}</Typography></Box>{version.status==='SCHEDULED'&&canEdit?<Button color="error" onClick={()=>cancel.mutate(version.id)}>Cancel scheduled change</Button>:null}</Stack></Paper></Grid>)}</>:null}
    </Grid></DialogContent><DialogActions><Button onClick={() => setOpen(false)}>Cancel</Button>{(!selected||canEdit)?<Button variant="contained" disabled={invalidRegisterPricing||schedule.isPending} onClick={() => selected ? (selected.status !== 'ACTIVE' || form.status !== selected.status ? update.mutate() : applicationMode==='IMMEDIATE'?setConfirmImmediate(true):schedule.mutate()) : save.mutate()}>{selected?'Publish Changes':'Create'}</Button>:null}</DialogActions></Dialog>
    <Dialog open={confirmImmediate} onClose={()=>setConfirmImmediate(false)} maxWidth="sm" fullWidth><DialogTitle>Apply Pricing Changes Immediately?</DialogTitle><DialogContent><Typography>This will make a new pricing plan version effective now for {selected?.activeMerchants??0} active merchant subscriptions on this plan.</Typography><Alert severity="warning" sx={{mt:2}}>Pricing, included limits, capabilities, and add-on availability may change immediately. Historical invoices will not be modified and no mid-cycle proration will be created.</Alert></DialogContent><DialogActions><Button onClick={()=>setConfirmImmediate(false)}>Cancel</Button><Button variant="contained" onClick={()=>schedule.mutate()} disabled={schedule.isPending}>Apply Immediately</Button></DialogActions></Dialog>
  </Stack>;
}
