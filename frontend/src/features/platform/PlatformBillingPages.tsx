...
export function PlatformPricingPlansPage() {
  ...
  return <Stack spacing={3}><Header title="Pricing Plans" action={<Button startIcon={<AddIcon />} variant="contained" onClick={openCreate}>New Plan</Button>} />
    ...
    </Grid></DialogContent><DialogActions><Button onClick={() => setOpen(false)}>Cancel</Button>{(!selected||canEdit)?<Button variant="contained" disabled={invalidRegisterPricing} onClick={() => selected?schedule.mutate():save.mutate()}>{selected?'Save Pricing Changes':'Create'}</Button>:null}</DialogActions></Dialog>
  </Stack>;
}
