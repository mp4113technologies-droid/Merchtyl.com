...
export function PlatformPricingPlansPage() {
  ...
  return <Stack spacing={3}><Header title="Pricing Plans" action={<Button startIcon={<AddIcon />} variant="contained" onClick={openCreate}>New Plan</Button>} />
    ...
    <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="md"><DialogTitle>{selected?'Edit Pricing':'Create Pricing Plan'}</DialogTitle><DialogContent><Grid container spacing={2} sx={{ mt: 0 }}>
      ...
      {selected?<><Grid item xs={12}><Typography variant="h6">Schedule Pricing Change</Typography></Grid><Grid item xs={6}><SelectField label="Effective" value={effectivePolicy} values={['NEXT_BILLING_CYCLE','SPECIFIC_FUTURE_DATE']} onChange={setEffectivePolicy}/></Grid><Grid item xs={6}>{effectivePolicy==='SPECIFIC_FUTURE_DATE'?<TextField fullWidth label="Effective Date" type="date" InputLabelProps={{shrink:true}} value={effectiveDate} onChange={e=>setEffectiveDate(e.target.value)}/>:null}</Grid><Grid item xs={12}><SelectField label="Existing Subscribers" value={subscriberPolicy} values={['NEW_SUBSCRIPTIONS_ONLY','APPLY_NEXT_BILLING_CYCLE']} onChange={setSubscriberPolicy}/></Grid><Grid item xs={12}><Alert severity="info">Pricing Change Summary: Base subscription {money(selected.basePrice,selected.currency)} → {money(Number(form.basePrice),selected.currency)}; additional store {money(selected.additionalStorePrice??0,selected.currency)} → {money(Number(form.additionalStorePrice??0),selected.currency)}. Affected active subscriptions: {selected.activeMerchants}.</Alert></Grid><Grid item xs={12}><FormControlLabel control={<Checkbox checked={confirmRemoval} onChange={e=>setConfirmRemoval(e.target.checked)}/>} label="Confirm capability removals for affected subscriptions at their next billing boundary" /></Grid><Grid item xs={12}><Typography variant="h6">Pricing History</Typography></Grid>{history.data?.map(version=><Grid item xs={12} key={version.id}><Paper variant="outlined" sx={{p:1}}><Stack direction="row" justifyContent="space-between"><Box><Typography>Version {version.versionNumber} · {version.status} · effective {version.effectiveFrom}</Typography><Typography variant="caption">Base {money(version.pricing.basePrice,version.pricing.currency)} · additional store {money(version.pricing.additionalStorePrice??0,version.pricing.currency)} · {version.subscriberPolicy.replaceAll('_',' ')}</Typography></Box>{version.status==='SCHEDULED'&&canEdit?<Button color="error" onClick={()=>cancel.mutate(version.id)}>Cancel scheduled change</Button>:null}</Stack></Paper></Grid>)}</>:null}
    </Grid></DialogContent><DialogActions><Button onClick={() => setOpen(false)}>Cancel</Button>{(!selected||canEdit)?<Button variant="contained" onClick={() => selected?schedule.mutate():save.mutate()}>{selected?'Save Pricing Changes':'Create'}</Button>:null}</DialogActions></Dialog>
  </Stack>;
}
